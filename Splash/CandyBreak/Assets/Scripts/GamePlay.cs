...
public class GamePlay : MonoBehaviour {
	...
	void Start () {
        ...
        // createNewTrap();
        float Board_y = GameObject.Find("bgTile_Board").GetComponent<SpriteRenderer>().bounds.size.y;
        Board_y = Board_y - Board_y / 50;
        TILE_Y = Board_y / 7;// GameObject.Find("bgTile_Board").GetComponent<SpriteRenderer>().bounds.size.y / 7;
        
        float Board_x = GameObject.Find("bgTile_Board").GetComponent<SpriteRenderer>().bounds.size.x ;
        Board_x = Board_x -Board_x / 60;
        TILE_X = Board_x/7;            
       
        BEGIN_X = -TILE_X * 7/2 +TILE_X/2;// -GameObject.Find("bgTile_Board").GetComponent<SpriteRenderer>().bounds.size.x / 2 + TILE_X / 2 + 2 * offsetx;        
        BEGIN_Y = 2.50f;// TILE_Y * 7 / 2 - TILE_Y / 2 - 0.40f;
        initgame();
